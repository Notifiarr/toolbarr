...
{#if form[idx][field] !== undefined}
  <td bind:this={cell} class={changed?"bg-warning":""}>
    <div class="{type} p-0">
      ...
      {:else if type == "text"}
      <div class="link" on:keyup={()=>{plaintext = false}} on:click={()=>{plaintext = false}}>
        <Input {plaintext} disabled={updating} invalid={changed} {type} bind:value={form[idx][field]}/>
      </div>
      {:else if type == "select"}
        <InputGroup size="sm">
...
<style>
  ...
  .select, .text { /* Smash that select into the table row. */
